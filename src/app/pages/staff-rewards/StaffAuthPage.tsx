...
export default function StaffAuthPage() {
  ...
  const runScan = async () => {
    if (staffId.trim().length < 4 || step !== "idle") return;
    setStep("starting");
    setProgress(0);
    try {
      ...
      // camera denied/unavailable — run a simulated scan so the flow still works
      setHasCamera(false);
  ...
  return (
    <MobileLayout>
      <div className="bg-white h-full flex flex-col">
        <StaffHeader title="Staff Sign-In" onBack={() => { stopStream(); navigate("/staff-rewards"); }} />
        <StaffProgressTracker currentStep={1} totalSteps={11} labels={journeyLabels} />

        <div className="flex-1 overflow-y-auto px-[24px] pt-[10px] pb-[120px]">
          <h2 className="font-['Effra',sans-serif] font-bold text-[20px] text-[#383838] mb-[6px]">Verify it's you</h2>
          <p className="font-['Effra',sans-serif] text-[14px] text-[#707070] leading-[20px] mb-[24px]">
            Sign in with your Staff ID and facial authentication before onboarding a customer.
          </p>
          ...
          <div className="mb-[24px]">
            <FloatingField
              label="Staff ID"
              value={staffId}
              onChange={(v) => setStaffId(v.toUpperCase())}
              hint="e.g. DSA-10472"
            />
          </div>
          ...
          <div className="flex flex-col items-center">
            <div className="relative">
              ...
              >
                {/* live front-camera feed, mirrored, clipped to the circle */}
                <video
                  ref={videoRef}
                  autoPlay
                  playsInline
        ...
        <div className="sticky bottom-0 left-0 right-0 bg-white border-t border-[#E2E8F0] p-[16px] z-20">
          <div className="max-w-[430px] mx-auto">
            {verified ? (
              <button onClick={() => navigate("/staff-rewards/category")} className={`${ctaCls} ${ctaEnabled}`}>
                Continue
              </button>
            ) : (
              <button
                onClick={runScan}
                disabled={staffId.trim().length < 4 || step !== "idle"}
                className={`${ctaCls} ${staffId.trim().length >= 4 && step === "idle" ? "bg-[#003883] text-white shadow-[0_8px_20px_rgba(0,56,131,0.30)] hover:bg-[#002a63] active:scale-[0.98]" : ctaDisabled}`}
              >
                {step === "idle" ? "Scan face" : "Verifying…"}
              </button>
            )}
          </div>
        </div>
